...
function getMeta(url){ID3.loadTags(url, function() {setMeta(url);},{tags: ["title","artist","album","picture"]});}function setMeta(url){var tags = ID3.getAllTags(url);meta.title = tags.title || "";meta.artist = tags.artist || "";meta.album = tags.album || "";var image = tags.picture;if(image){var base64String = "";for(var i=0;i<image.data.length;i++){base64String+=String.fromCharCode(image.data[i]);}var base64="data:"+image.format+";base64,"+window.btoa(base64String);meta.albumCover = base64;}else{meta.albumCover = 'files/img/young_shubham.jpg';}addMetaToUI();}
function addMetaToUI(){/*//trackTitle.innerHTML = '<span class="centroid" > '+meta.title+' </span>';//album.innerHTML = meta.album;*/artist.innerHTML = '<span class="centroid" > '+meta.artist+' </span>';albumArt.style.display ='block';albumArt.src = meta.albumCover;trackMetaData.innerHTML='Title : '+meta.title+'<br><br>'+'Artist : '+meta.artist+'<br><br>'+'Album : '+meta.album+'<br>';}
var meta={album:undefined,albumCover:undefined,artist:undefined,title:undefined}
